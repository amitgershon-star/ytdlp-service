...
PROXY_URL = os.environ.get("PROXY_URL", "")          # e.g. http://user:pass@proxy.webshare.io:80
INSTAGRAM_COOKIES_B64 = os.environ.get("INSTAGRAM_COOKIES_B64", "")  # base64-encoded Netscape cookie file
...
def download():
    ...
    # YouTube at 720p can be 100-200 MB — far over the 15 MB base64 limit in the webhook.
    # Keep YouTube at 480p max so short recipe clips stay under 15 MB for Gemini inline video.
    if platform == "youtube":
        cmd.extend(["--format",
    ...
    # Route through residential proxy when configured — bypasses data center IP blocks.
    # Required for YouTube yt-dlp (YouTube blocks Render's IP without a proxy).
    if PROXY_URL:
    ...
    if INSTAGRAM_COOKIES_B64 and platform in ("instagram", "youtube", "facebook"):
        try:
            cookies_data = base64.b64decode(INSTAGRAM_COOKIES_B64).decode("utf-8")
...
def transcript():
    ...
    Works from data center IPs (different mechanism than yt-dlp video download).
    Returns the full spoken transcript — far better than description for recipe videos.
    """
    data = request.get_json(silent=True)
    if not data or not data.get("url"):
    ...
    try:
        ...
        try:
            t = transcript_list.find_transcript(["en", "en-US", "en-GB", "he"])
        except Exception:
            generated = transcript_list._generated_transcripts
            if not generated:
                return jsonify({"transcript": None, "error": "No captions available"})
...
def proxy_fetch():
    """
    HTTP proxy endpoint — fetches a URL from Render's IP (different from Supabase's
    Cloudflare IP range). Optionally routes through PROXY_URL if configured.

    Use for: Instagram ?__a=1 API calls that are rate-limited on Supabase's IP.
